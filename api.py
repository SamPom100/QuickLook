...
def get_shares_outstanding(ticker: str) -> float:
    url = f'https://www.alphavantage.co/query?function=OVERVIEW&symbol={ticker}&apikey=YOUR_ALPHA_KEY_0'
    ...
    if cached:
        ...
        overview = resp.json()
        if "Information" not in overview and "Note" not in overview:
            cm.save_url_cache(url, json.dumps(overview))
    return float(overview.get('SharesOutstanding', 0)) if isinstance(overview, dict) else 0.0
...
def get_data(ticker):
    ...
    # Historical P/E calculation per quarter
    ...
    for i, q in enumerate(quarters):
        ...
        q_dt = q_dates[i]
        mask_p = (p_dates >= q_dt)
        q_price = prices[mask_p][0] if mask_p.any() else latest_price
